...
export const MatchItem: FunctionComponent<{
  match: Match;
  onFinishMatch: (id: string) => void;
  onUpdateScore: (id: string, home: number, away: number) => void
}> = ({ match, onFinishMatch, onUpdateScore }) => {
  const handleScoreChange = (teamId: TeamId) => (e: React.ChangeEvent<HTMLInputElement>) => {
    const value = parseInt(e.target.value, 10) || 0;
    onUpdateScore(
      match.matchId,
      teamId === TeamId.HOME ? value : match.homeTeam.score,
      teamId === TeamId.AWAY ? value : match.awayTeam.score
    );
  };
    ...
    <li>
      ...
      <div>
        <input
          type="number"
          step="1"
          min={match.homeTeam.score}
          placeholder="0"
          value={match.homeTeam.score}
          onChange={handleScoreChange(TeamId.HOME)}
        />
        <input
          type="number"
          step="1"
          min={match.awayTeam.score}
          placeholder="0"
          value={match.awayTeam.score}
          onChange={handleScoreChange(TeamId.AWAY)}
        />
